# main.py
# Canonical 10-client run that reproduces the results reported in Table 4 / Table 6
# of the paper.  Uses the leak-free evaluation protocol: D_val for the Zero-Trust
# mechanism only; all final metrics are computed on the held-out D_test.
...
# ----------------------
# Hyperparameters (paper-default for the 10-client run)
# ----------------------
NUM_CLIENTS = 10
NUM_ROUNDS = 20
LOCAL_EPOCHS = 5
...
trainloaders, valloader, testloader, trainloader_centralized, input_size, num_classes, class_names = prepare_data(
    csv_path=CSV_PATH, num_clients=NUM_CLIENTS, beta=0.5, seed=42
)
...
# Export Results
# ----------------------
with pd.ExcelWriter(os.path.join(RESULTS_DIR, "publication_results.xlsx")) as writer:
    results_data = {
...
print(f"Results saved to {os.path.join(RESULTS_DIR, 'publication_results.xlsx')}")
...
plt.savefig(os.path.join(FIG_DIR, "publication_results.png"), dpi=300)
print(f"Figures saved to {os.path.join(FIG_DIR, 'publication_results.png')}")
